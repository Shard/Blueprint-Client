...
namespace Blueprint
{
    class WeaponPackage
    {
        ...
        public void Update(Control control, Camera camera, Player player, NpcPackage npcs, Ui.FloatingTextCollection floatingText)
        {
            ...
            {
                if (player.Inventory.Quickbar.UsingItem.Value == "11" && !player.Inventory.Quickbar.IsUsingItem)
                {
                    LiveWeapons.Add(new LiveWeapon(GetWeaponById(player.Inventory.Quickbar.UsingItem.IntValue), startingSwordPosition(player, new Rectangle(0, 0, 47, 79)), player.Movement.Direction));
